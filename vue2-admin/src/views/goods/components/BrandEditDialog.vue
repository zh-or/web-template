<template>
    <el-dialog :visible.sync="show" :title="!obj ? '新建品牌' : '编辑品牌'" v-loading="loading" width="600px">
        <el-form :model="data.form"
                 :rules="data.rules"
                 label-width="120px"
                 ref="form"
                 :disabled="props.disabled"
                 class="dialog-form">

            <el-form-item label="品牌名称:" prop="brandName">
                <el-input v-model="data.form.brandName" maxlength="100" :show-word-limit="true" placeholder="请输入品牌名称"/>
            </el-form-item>
            <el-form-item label="排序值:" prop="iorder">
                <el-input v-model="data.form.iorder" type="number" placeholder="请输入排序值"/>
            </el-form-item>
            <el-form-item label="品牌logo:">
                <FileSelect :disabled="props.disabled"
                            v-model:fileList="data.form.logoFile"
                            :limit="1"
                            :isPic="true"/>
            </el-form-item>
            <el-form-item label="备注:" prop="remark">
                <el-input v-model="data.form.remark" type="textarea" :rows="4" maxlength="200" :show-word-limit="true" placeholder="请输入备注"/>
            </el-form-item>
        </el-form>
        <template #footer>
            <div class="dialog-footer" v-if="!props.disabled">
                <el-button type="primary" @click="confirmEditFun">确 定</el-button>
                <el-button @click="show = false">取 消</el-button>
            </div>
            <div class="dialog-footer" v-else>
                <el-button type="primary" @click="show = false">确 定</el-button>
            </div>
        </template>
    </el-dialog>
</template>

<script setup name="brand-dialog">
    import {getRule} from "@/utils/utils";

    import {
        $api,
        $modal,
        $notify,
        $store,
        $router,
        getImgUrl,
        downloadXhr,
    } from '@/autoImportUtils/index.js';
    import {
        obj2FormData
    } from "@/utils/utils";

    const emit = defineEmits(['update']);

    const props = defineProps({
        visible: Boolean,
        obj: Object,
        disabled: {
            type: Boolean,
            default: false
        }
    })
    const show = ref(false);
    const loading = ref(false);
    const form = ref(null);
    const data = reactive({
        form: {
            brandName: '',
            iorder: '',
            logoFile: null,
            remark:'',
        },
        rules: {
            brandName: getRule('请输入品牌名称'),
            iorder: getRule('请输入排序值'),
        }
    });
    show.value = !!props.visible.value;

    watch(show, _ => {
        emit('update:visible', _);
    })

    watch(() => props.visible, _ => {
        show.value = _;
        if(_) {
            if (props.obj) {
                data.form.brandName = props.obj.brandName;
                data.form.iorder = props.obj.iorder;
                data.form.remark = props.obj.remark;
                data.form.logoFile = props.obj.logoFile;/*{
                    isDel: false,
                    isNew: false,
                    file: null,
                    url: props.obj.logo,
                    name: props.obj.logo,
                    id: '',
                };*/
            } else {

                data.form.brandName = '';
                data.form.iorder = '';
                data.form.remark = '';
                data.form.logoFile = null;
            }
        }
    })

    function confirmEditFun() {
        form.value.validate(async _ => {
            if (_) {
                try {
                    loading.value = true;
                    let form = {
                        brandName: data.form.brandName,
                        iorder: data.form.iorder,
                        remark: data.form.remark,
                    }
                    console.log('a:', toRaw(data.form));
                    if (data.form.logoFile && data.form.logoFile.isNew) {
                        form.logoFile = data.form.logoFile.file;
                    }
                    form = obj2FormData(form);
                    if (props.obj) {
                        form.append('brandId', props.obj.brandId);
                        await $api.editBrand(form);
                    } else {
                        await $api.addBrand(form);
                    }
                    emit('update');
                    $modal.success('操作成功');
                    loading.value = false;
                    show.value = false;
                } catch(e) {
                    loading.value = false;
                    console.error(e);
                }
            }
        })
    }
</script>

<style lang="scss">

</style>
