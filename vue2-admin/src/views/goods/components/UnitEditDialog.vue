<template>
    <el-dialog :visible.sync="show" :title="!obj ? '新建单位' : '编辑单位'" v-loading="loading" width="600px">
        <el-form :model="data.form"
                 :rules="data.rules"
                 label-width="120px"
                 ref="form"
                 :disabled="props.disabled"
                 class="dialog-form">

            <el-form-item label="单位名称:" prop="unitName">
                <el-input v-model="data.form.unitName" maxlength="100" :show-word-limit="true" placeholder="请输入单位名称"/>
            </el-form-item>
            <el-form-item label="排序值:" prop="iorder">
                <el-input v-model="data.form.iorder" type="number" placeholder="请输入排序值"/>
            </el-form-item>

            <el-form-item label="备注:" prop="remark">
                <el-input v-model="data.form.remark" type="textarea" :rows="4" maxlength="200" :show-word-limit="true" placeholder="请输入备注"/>
            </el-form-item>
        </el-form>
        <template #footer>
            <div class="dialog-footer" v-if="!props.disabled">
                <el-button type="primary" @click="confirmEditFun">确 定</el-button>
                <el-button @click="show = false">取 消</el-button>
            </div>
            <div class="dialog-footer" v-else>
                <el-button type="primary" @click="show = false">确 定</el-button>
            </div>
        </template>
    </el-dialog>
</template>

<script setup name="brand-dialog">
    import {getRule} from "@/utils/utils";

    import {
        $api,
        $modal,
        $notify,
        $store,
        $router,
        getImgUrl,
        downloadXhr,
    } from '@/autoImportUtils/index.js';

    const emit = defineEmits(['upload']);
    const form = ref(null);
    const props = defineProps({
        visible: Boolean,
        obj: Object,
        disabled: {
            type: Boolean,
            default: false
        }
    })
    const show = ref(false);
    const loading = ref(false);
    const data = reactive({
        form: {
            unitName: '',
            iorder: '',
            remark:'',
        },
        rules: {
            unitName: getRule('请输入单位名称'),
            iorder: getRule('请输入排序值'),
        }
    });
    show.value = !!props.visible.value;

    watch(show, _ => {
        emit('update:visible', _);
    })

    watch(() => props.visible, _ => {
        show.value = _;
        if(_) {
            if (props.obj) {
                data.form.unitName = props.obj.unitName;
                data.form.iorder = props.obj.iorder;
                data.form.remark = props.obj.remark;
            } else {

                data.form.unitName = '';
                data.form.iorder = '';
                data.form.remark = '';
            }
        }
    })

    function confirmEditFun() {
        form.value.validate(async _ => {
            if (_) {
                try {
                    loading.value = true;
                    let form = {
                        unitName: data.form.unitName,
                        iorder: data.form.iorder,
                        remark: data.form.remark,
                        unitId: props.obj?.unitId || '',
                    }
                    if (props.obj) {
                        await editUnit(form);
                    } else {
                        await addUnit(form);
                    }
                    emit('update');
                    $modal.success('操作成功');
                    loading.value = false;
                    show.value = false;
                } catch(e) {
                    console.error(e);
                    loading.value = false;
                }
            }
        })
    }
</script>

<style lang="scss">

</style>
